...
class Woff2Dec {
    ...
    private static int tripletDecode(byte[] data, int flags_in_offset, int in_offset, int in_size, int n_points, Woff2Common.Point[] result) {
        ...
        for (int i = 0; i < n_points; ++i) {
            ...
            if (flag < 10) {
                ...
                dy = withSign(flag >> 1,
                        1 + (((b0 % 12) >> 2) << 8) + JavaUnsignedUtil.asU8(data[in_offset + triplet_index + 1]));
            } else if (flag < 124) {
                ...
                dx = withSign(flag, (JavaUnsignedUtil.asU8(data[in_offset + triplet_index]) << 8) + JavaUnsignedUtil.asU8(data[in_offset + triplet_index + 1]));
                dy = withSign(flag >> 1,
                        (JavaUnsignedUtil.asU8(data[in_offset + triplet_index + 2]) << 8) + JavaUnsignedUtil.asU8(data[in_offset + triplet_index + 3]));
            }
    ...
    private static int storeOffsetTable(byte[] result, int offset, int flavor, int num_tables) {
        offset = StoreBytes.storeU32(result, offset, flavor);  // sfnt version
        offset = StoreBytes.storeU16(result, offset, num_tables);  // num_tables
        int max_pow2 = 0;
        while (1 << (max_pow2 + 1) <= num_tables) {
            max_pow2++;
        }
        int output_search_range = (1 << max_pow2) << 4;
        offset = StoreBytes.storeU16(result, offset, output_search_range);  // searchRange
        offset = StoreBytes.storeU16(result, offset, max_pow2);  // entrySelector
        // rangeShift
    ...
    private static long computeOffsetToFirstTable(Woff2Header hdr) {
        long offset = Woff2Common.kSfntHeaderSize +
                Woff2Common.kSfntEntrySize * hdr.num_tables;
        if (hdr.header_version != 0) {
            offset = Woff2Common.collectionHeaderSize(hdr.header_version, hdr.ttc_fonts.length)
                    + Woff2Common.kSfntHeaderSize * hdr.ttc_fonts.length;
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            if (!reused) {
                if ((table.flags & Woff2Common.kWoff2FlagsTransform) != Woff2Common.kWoff2FlagsTransform) {
                    ...
                    if (table.tag == TableTags.kGlyfTableTag) {
                    ...
                    } else if (table.tag == TableTags.kLocaTableTag) {
                        // All the work was done by reconstructGlyf. We already know checksum.
                        checksum = loca_checksum;
                    } else if (table.tag == TableTags.kHmtxTableTag) {
                        table.dst_offset = dest_offset;
                        ...
                        checksum = reconstructTransformedHmtx(transformed_buf,
                                ...
                                JavaUnsignedUtil.asU16(info.num_glyphs), JavaUnsignedUtil.asU16(info.num_hmetrics), info.x_mins,
                                out);
                    } else {
    ...
    private static void writeHeaders(byte[] data, int length, RebuildMetadata metadata,
        ...
        if (hdr.header_version != 0) {
            // TTC header
            offset = StoreBytes.storeU32(result, offset, hdr.flavor);  // TAG TTCTag
            offset = StoreBytes.storeU32(result, offset, hdr.header_version);  // FIXED Version
            offset = StoreBytes.storeU32(result, offset, hdr.ttc_fonts.length);  // ULONG numFonts
            ...
            for (int i = 0; i < hdr.ttc_fonts.length; ++i) {
                offset = StoreBytes.storeU32(result, offset, 0);  // will fill real values in later
            }
            // space for DSIG fields for header v2
            if (hdr.header_version == 0x00020000) {
                offset = StoreBytes.storeU32(result, offset, 0);  // ULONG ulDsigTag
                offset = StoreBytes.storeU32(result, offset, 0);  // ULONG ulDsigLength
                offset = StoreBytes.storeU32(result, offset, 0);  // ULONG ulDsigOffset
            }
