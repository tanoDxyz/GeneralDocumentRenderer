...
public abstract class PdfSimpleFont<T extends FontProgram> extends PdfFont {
     ...
     */
    protected byte[] usedGlyphs = new byte[SIMPLE_FONT_MAX_CHAR_CODE_VALUE + 1];
    ...
    protected void flushFontData(String fontName, PdfName subtype) {
        ...
        int firstChar;
        int lastChar;
        for (firstChar = 0; firstChar <= SIMPLE_FONT_MAX_CHAR_CODE_VALUE; ++firstChar) {
            if (usedGlyphs[firstChar] != 0) break;
        }
        for (lastChar = SIMPLE_FONT_MAX_CHAR_CODE_VALUE; lastChar >= firstChar; --lastChar) {
            if (usedGlyphs[lastChar] != 0) break;
        }
        if (firstChar > SIMPLE_FONT_MAX_CHAR_CODE_VALUE) {
            firstChar = SIMPLE_FONT_MAX_CHAR_CODE_VALUE;
            lastChar = SIMPLE_FONT_MAX_CHAR_CODE_VALUE;
        }
