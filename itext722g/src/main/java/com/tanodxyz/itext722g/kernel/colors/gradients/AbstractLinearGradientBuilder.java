 ...
 */
package com.tanodxyz.itext722g.kernel.colors.gradients;
...
import com.tanodxyz.itext722g.kernel.pdf.colorspace.PdfShading;
...
public abstract class AbstractLinearGradientBuilder {
    ...
    private static PdfShading.Axial createAxialShading(Point[] baseCoordinatesVector,
        ...
        if (baseVectorLength < ZERO_EPSILON || stopsToConstruct.size() == 1) {
            ...
            stopsToConstruct = Arrays.asList(new GradientColorStop(lastColorStop, 0d, GradientColorStop.OffsetType.RELATIVE),
                    new GradientColorStop(lastColorStop, 1d, GradientColorStop.OffsetType.RELATIVE));
        } else {
    ...
    private static List<GradientColorStop> normalizeStops(List<GradientColorStop> toNormalize, double baseVectorLength) {
        if (baseVectorLength < ZERO_EPSILON) {
            return Arrays.asList(new GradientColorStop(toNormalize.get(toNormalize.size() - 1),
                    0d, GradientColorStop.OffsetType.RELATIVE));
        }
    ...
    private static void normalizeHintsOffsets(List<GradientColorStop> result) {
        ...
        result.get(result.size() - 1).setHint(0, GradientColorStop.HintOffsetType.NONE);
    }
    ...
    private static void normalizeAutoStops(List<GradientColorStop> toNormalize) {
        ...
        for (int i = 1; i < toNormalize.size(); ++i) {
            ...
            if (currentStop.getOffsetType() == GradientColorStop.OffsetType.AUTO) {
                ...
                firstAutoStopIndex = i + 1;
                prevOffset = currentStop.getHintOffsetType() == GradientColorStop.HintOffsetType.RELATIVE_ON_GRADIENT
    ...
    private static List<GradientColorStop> adjustNormalizedStopsToCoverDomain(List<GradientColorStop> normalizedStops,
        ...
        if (originalIntervalLength <= ZERO_EPSILON) {
            return Arrays.asList(new GradientColorStop(lastColorStop, targetDomain[0], GradientColorStop.OffsetType.RELATIVE),
                    new GradientColorStop(lastColorStop, targetDomain[1], GradientColorStop.OffsetType.RELATIVE));
        }
