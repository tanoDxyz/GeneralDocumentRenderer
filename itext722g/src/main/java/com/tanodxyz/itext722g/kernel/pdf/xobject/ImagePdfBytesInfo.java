...
import com.tanodxyz.itext722g.io.codec.PngWriter;
import com.tanodxyz.itext722g.io.codec.TIFFConstants;
import com.tanodxyz.itext722g.io.codec.TiffWriter;
...
class ImagePdfBytesInfo {
    ...
    public byte[] decodeTiffAndPngBytes(byte[] imageBytes) throws IOException {
        ...
        if (pngColorType < 0) {
            if (bpc != 8)
                throw new com.tanodxyz.itext722g.io.exceptions.IOException(com.tanodxyz.itext722g.io.exceptions.IOException.ColorDepthIsNotSupported).setMessageParams(bpc);

            if (colorspace instanceof PdfArray) {
                PdfArray ca = (PdfArray) colorspace;
                PdfObject tyca = ca.get(0);
                if (!PdfName.ICCBased.equals(tyca))
                    throw new com.tanodxyz.itext722g.io.exceptions.IOException(com.tanodxyz.itext722g.io.exceptions.IOException.ColorSpaceIsNotSupported).setMessageParams(tyca.toString());
                PdfStream pr = (PdfStream) ca.get(1);
                int n = pr.getAsNumber(PdfName.N).intValue();
                if (n != 4) {
                    throw new com.tanodxyz.itext722g.io.exceptions.IOException(com.tanodxyz.itext722g.io.exceptions.IOException.NValueIsNotSupported).setMessageParams(n);
                }
                icc = pr.getBytes();
            } else if (!PdfName.DeviceCMYK.equals(colorspace)) {
                throw new com.tanodxyz.itext722g.io.exceptions.IOException(com.tanodxyz.itext722g.io.exceptions.IOException.ColorSpaceIsNotSupported).setMessageParams(colorspace.toString());
            }
            stride = 4 * width;
